...
AdminCli::AdminCli(AdminService &adminService, BookService &bookService,
                   MemberService &memberService, LoanService &loanService)
    : adminService_(adminService), bookService_(bookService),
      memberService_(memberService), loanService_(loanService) {}
...
bool AdminCli::login() {
  ...
  for (int attempt = 1; attempt <= maxAttempts; ++attempt) {
    ...
    std::cout << "[ERROR] Invalid username or password. Attempt " << attempt
              << "/" << maxAttempts << ".\n\n";
  }
...
void AdminCli::editBook() {
  ...
  std::string newAuthor =
      readLine("New author (leave empty to keep current): ");
...
void AdminCli::deleteBook() {
  ...
  if (bookService_.deleteBook(id)) {
    ...
    std::cout << "\n[ERROR] Failed to delete book. The book may have an active "
                 "loan.\n";
  }
...
void AdminCli::addMember() {
  ...
  if (memberId.empty() || username.empty() || password.empty() ||
  ...
  Member member(memberId, username, PasswordHasher::hash(password), name,
                email);
...
void AdminCli::deleteMember() {
  ...
  if (memberService_.deleteMember(memberId)) {
    ...
    std::cout << "\n[ERROR] Failed to delete member. The member may have an "
                 "active loan.\n";
  }
...
void AdminCli::issueLoan() {
  ...
  if (loanService_.borrowBook(bookId, memberId, borrowDate, dueDate)) {
    ...
    std::cout << "\n[ERROR] Failed to issue loan. Check book availability and "
                 "member ID.\n";
  }
...
void AdminCli::returnLoan() {
  ...
  std::string nextDueDate =
      readLine("Next due date if reserved (YYYY-MM-DD): ");
  ...
  if (loanService_.returnBook(loanId, returnDate, nextDueDate)) {
    ...
    std::cout << "[INFO] If the book had a reservation queue, it was assigned "
                 "automatically.\n";
  } else {
    std::cout
...
void AdminCli::reserveBook() {
  ...
  if (loanService_.reserveBook(bookId, memberId, reservedAt)) {
    ...
    std::cout << "\n[ERROR] Failed to reserve book. Check book ID, member ID, "
                 "or duplicate reservation.\n";
  }
