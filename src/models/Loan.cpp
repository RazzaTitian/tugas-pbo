...
Loan::Loan(
    ...
    : loanId_(loanId),
      ...
      borrowDate_(std::move(borrowDate)),
...
int Loan::bookId() const {
    return bookId_;
}

const std::string& Loan::memberId() const {
    return memberId_;
}

const std::string& Loan::borrowDate() const {
    return borrowDate_;
}

const std::string& Loan::dueDate() const {
    return dueDate_;
}

const std::string& Loan::returnDate() const {
    return returnDate_;
}
...
void Loan::setStatus(LoanStatus status) {
    status_ = status;
}

bool Loan::isActive() const {
    return status_ == LoanStatus::Active;
}

bool Loan::isReturned() const {
    return status_ == LoanStatus::Returned;
}
...
std::ostream& operator<<(std::ostream& os, const Loan& loan) {
    os << "Loan{id=" << loan.loanId_
       << ", bookId=" << loan.bookId_
       << ", memberId=\"" << loan.memberId_
       << "\", borrowDate=\"" << loan.borrowDate_
       << "\", dueDate=\"" << loan.dueDate_
