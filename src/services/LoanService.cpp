...
LoanService::LoanService(CsvBookRepository &bookRepository,
...
bool LoanService::borrowBook(int bookId, const std::string &memberId,
  ...
  Loan loan(loanRepository_.nextId(), bookId, memberId, borrowDate, dueDate, "",
            LoanStatus::Active);
...
bool LoanService::returnBook(int loanId, const std::string &returnDate,
  ...
  auto reservationOpt =
      reservationRepository_.findFirstWaitingByBookId(book.id());
  ...
  if (reservationOpt.has_value()) {
    ...
    Loan nextLoan(loanRepository_.nextId(), book.id(), reservation.memberId(),
                  returnDate, nextDueDate, "", LoanStatus::Active);
...
bool LoanService::reserveBook(int bookId, const std::string &memberId,
  ...
  for (const Reservation &reservation :
       reservationRepository_.listWaitingByBookId(bookId)) {
    if (reservation.memberId() == memberId) {
      return false;
    }
  }

  Reservation reservation(reservationRepository_.nextId(), bookId, memberId,
                          reservedAt, ReservationStatus::Waiting);
...
std::vector<Loan>
LoanService::listLoanHistoryByMember(const std::string &memberId) const {
