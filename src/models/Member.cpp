...
const std::string& Member::memberId() const {
    return memberId_;
}
...
const std::string& Member::email() const {
    return email_;
}
...
std::ostream& operator<<(std::ostream& os, const Member& member) {
    os << "Member{id=" << member.memberId_
       << ", username=" << member.username_
       << ", name=\"" << member.name_
       << "\", email=\"" << member.email_
       << "\"}";
    return os;
