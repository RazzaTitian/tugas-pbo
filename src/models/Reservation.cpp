...
Reservation::Reservation()
    : reservationId_(0), bookId_(0), memberId_(""), reservedAt_(""),
      status_(ReservationStatus::Waiting) {}

Reservation::Reservation(int reservationId, int bookId, std::string memberId,
                         std::string reservedAt, ReservationStatus status)
    : reservationId_(reservationId), bookId_(bookId),
      memberId_(std::move(memberId)), reservedAt_(std::move(reservedAt)),
      status_(status) {}

int Reservation::reservationId() const { return reservationId_; }

int Reservation::bookId() const { return bookId_; }

const std::string &Reservation::memberId() const { return memberId_; }

const std::string &Reservation::reservedAt() const { return reservedAt_; }
...
std::string Reservation::statusText() const { return statusToString(status_); }
...
std::ostream &operator<<(std::ostream &os, const Reservation &reservation) {
  os << "Reservation{id=" << reservation.reservationId_
     << ", bookId=" << reservation.bookId_ << ", memberId=\""
     << reservation.memberId_ << "\", reservedAt=\"" << reservation.reservedAt_
