...
Admin::Admin(std::string username, std::string passwordHash, std::string displayName)
    : User(std::move(username), std::move(passwordHash)),
      displayName_(std::move(displayName)) {}

const std::string& Admin::displayName() const {
    return displayName_;
}
...
std::ostream& operator<<(std::ostream& os, const Admin& admin) {
    os << "Admin{username=" << admin.username_
       << ", displayName=\"" << admin.displayName_
       << "\"}";
    return os;
}