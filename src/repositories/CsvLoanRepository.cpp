...
Loan CsvLoanRepository::fromCsvLine(const std::string &line) {
  ...
  return Loan(std::stoi(loanIdStr), std::stoi(bookIdStr), memberId, borrowDate,
              dueDate, returnDate, Loan::statusFromString(status));
}

std::string CsvLoanRepository::toCsvLine(const Loan &loan) {
  std::stringstream ss;

  ss << loan.loanId() << "," << loan.bookId() << "," << loan.memberId() << ","
     << loan.borrowDate() << "," << loan.dueDate() << "," << loan.returnDate()
     << "," << loan.statusText();
...
std::vector<Loan>
CsvLoanRepository::listByMemberId(const std::string &memberId) const {
...
std::vector<Loan>
CsvLoanRepository::listOverdue(const std::string &today) const {
